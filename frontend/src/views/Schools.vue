...
const editSchool = (school) => {
  ...
  form.value = {
    ...
    domain: school.domains?.[0]?.domain?.replace(APP_DOMAIN, '') || '',
    contact_email: school.contact_email || '',
    contact_phone: school.contact_phone || ''
  }
  logoPreview.value = school.logo ? `${API_URL}/storage/${school.logo}` : null
  logoFile.value = null
...
const saveSchool = async () => {
  try {
    ...
    data.append('domain', (form.value.domain || '') + APP_DOMAIN)
    ...
    if (isEditing.value) {
      ...
      if (response.data.admin_email && response.data.admin_password) {
        newSchoolCredentials.value = {
          ...
          domain: form.value.domain + APP_DOMAIN
        }
      }
    }
          ...
          <form @submit.prevent="saveSchool" class="space-y-4">
            ...
            <div class="space-y-1">
              ...
              <div class="relative flex items-center">
                ...
                <span class="absolute left-3 text-[10px] font-black text-slate-300" dir="ltr">{{ APP_DOMAIN }}</span>
              </div>
            </div>
